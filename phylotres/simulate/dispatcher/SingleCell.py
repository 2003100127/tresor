        ...
        for pn in range(self.permutation_num):
            simu_params = {
                'init_seq_setting': {
                    ...
                    'is_seed': True,
                    'working_dir': to('data/simu/monomer/sc/pcr_num/permute_') + str(pn) + '/',
                    'is_sv_umi_lib': True,
                    'umi_lib_fpn': to('data/simu/monomer/sc/pcr_num/permute_') + str(pn) + '/',
                    # 'is_sv_seq_lib': True,
                    # 'seq_lib_fpn': to('data/simu/monomer/sc/pcr_num/permute_') + str(pn) + '/seq.txt',
                    'condis': ['umi'],
                ...
                'seed': None,
                'write': {
                    'fastq_fp': to('data/simu/monomer/sc/pcr_num/permute_') + str(pn) + '/',
                    'fastq_fn': '',
                }
            }
        ...
        for pn in range(self.permutation_num):
            simu_params = {
                'init_seq_setting': {
                    ...
                    # 'seq_len': self.seq_len_fixed - self.umi_unit_len_fixed,
                    'is_seed': True,
                    'working_dir': to('data/simu/monomer/sc/pcr_err/permute_') + str(pn) + '/',
                    'is_sv_umi_lib': True,
                    'umi_lib_fpn': to('data/simu/monomer/sc/pcr_err/permute_') + str(pn) + '/',
                    # 'is_sv_seq_lib': True,
                    # 'seq_lib_fpn': to('data/simu/monomer/sc/pcr_err/permute_') + str(pn) + '/seq.txt',
                    'condis': ['umi'],
                    'sim_thres': self.sim_thres_fixed,
                    'permutation': pn,
                },
                ...
                'seed': None,
                'write': {
                    'fastq_fp': to('data/simu/monomer/sc/pcr_err/permute_') + str(pn) + '/',
                    'fastq_fn': '',
                }
            }
            p = simusc(simu_params)
        ...
        for pn in range(self.permutation_num):
            simu_params = {
                'init_seq_setting': {
                    ...
                    # 'seq_len': self.seq_len_fixed - self.umi_unit_len_fixed,
                    'is_seed': True,
                    'working_dir': to('data/simu/monomer/sc/seq_err/permute_') + str(pn) + '/',
                    'is_sv_umi_lib':True,
                    'umi_lib_fpn':to('data/simu/monomer/sc/seq_err/permute_') + str(pn) + '/',
                    # 'is_sv_seq_lib':True,
                    # 'seq_lib_fpn':to('data/simu/monomer/sc/seq_err/permute_') + str(pn) + '/seq.txt',
                    'condis': ['umi'],
                    'sim_thres': self.sim_thres_fixed,
                    'permutation': pn,
                },
                ...
                'seed': None,
                'write': {
                    'fastq_fp': to('data/simu/monomer/sc/seq_err/permute_') + str(pn) + '/',
                    'fastq_fn': '',
                }
            }
            p = simusc(simu_params)
        ...
        for pn in range(self.permutation_num):
            simu_params = {
                'init_seq_setting': {
                    ...
                    'is_seed': True,
                    'working_dir': to('data/simu/monomer/sc/umi_len/permute_') + str(pn) + '/',
                    'is_sv_umi_lib': True,
                    'umi_lib_fp': to('data/simu/monomer/sc/umi_len/permute_') + str(pn) + '/',
                    # 'is_sv_seq_lib': True,
                    # 'seq_lib_fpn': to('data/simu/monomer/sc/umi_len/permute_') + str(pn) + '/',
                    'condis': ['umi'],
                    'sim_thres': 1,
                    # 'sim_thres': self.sim_thres_fixed,
                    'permutation': pn,
                ...
                'seed': None,
                'write': {
                    'fastq_fp': to('data/simu/monomer/sc/umi_len/permute_') + str(pn) + '/',
                    'fastq_fn': '',
                }
            }
            p = simusc(simu_params)
        ...
        for pn in range(self.permutation_num):
            simu_params = {
                'init_seq_setting': {
                    ...
                    # 'seq_len': self.seq_len_fixed - self.umi_unit_len_fixed,
                    'is_seed': True,
                    'working_dir': to('data/simu/monomer/sc/ampl_rate/permute_') + str(pn) + '/',
                    'is_sv_umi_lib': True,
                    'umi_lib_fpn': to('data/simu/monomer/sc/ampl_rate/permute_') + str(pn) + '/',
                    # 'is_sv_seq_lib': True,
                    # 'seq_lib_fpn': to('data/simu/monomer/sc/ampl_rate/permute_') + str(pn) + '/seq.txt',
                    'condis': ['umi'],
                    'sim_thres': self.sim_thres_fixed,
                    'permutation': pn,
                },
                ...
                'seed': None,
                'write': {
                    'fastq_fp': to('data/simu/monomer/sc/ampl_rate/permute_') + str(pn) + '/',
                    'fastq_fn': '',
                }
            }
            p = simusc(simu_params)
